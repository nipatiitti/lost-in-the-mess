<script>
  ...
  import { MessageSquare, Image as ImageIcon, Video, Send } from '@lucide/svelte';
  ...
  async function captureAndSend() {
    ...
    try {
      const res = await fetch('/api/video/frame', {
        ...
        body: JSON.stringify({ width: CAPTURE_WIDTH, height: CAPTURE_HEIGHT, data: b64 })
      });
      if (res.ok) {
        frameCount++;
      }
  ...
  function handleSend() {
    if (kind === 'video') {
      if (streaming) {
        stopStreaming();
        lastResult = { result: "OK", message: `Stream stopped. ${frameCount} frames sent.` };
...
<div style="padding:20px 24px;overflow:auto;flex:1;display:grid;grid-template-columns:1fr 360px;gap:20px">
  <div>
    ...
    <div style="margin-bottom:18px">
      ...
      {:else if kind === "image"}
        <div style="position:relative">
          ...
          <div style="border:1px dashed {imagePreview ? 'var(--signal-300)' : 'var(--border-strong)'};border-radius:2px;padding:32px 14px;text-align:center;color:var(--bone-400);background:var(--ink-100)">
            {#if imagePreview}
              <img src={imagePreview} alt="preview" style="max-height:320px;max-width:100%;border-radius:2px;margin:0 auto;display:block;margin-bottom:12px" />
      ...
      {:else if kind === "video"}
        <div style="border:1px solid {streaming ? 'var(--signal-300)' : 'var(--border)'};border-radius:2px;overflow:hidden;background:#000;position:relative">
          ...
          {#if streaming}
            ...
            ">
              ...
              <span style="font-family:var(--font-mono);font-size:10px;letter-spacing:0.12em;color:var(--lost-300)">
    ...
    <div style="display:flex;gap:10px;align-items:center">
      <button class="btn btn-primary" on:click={handleSend}>
        {#if kind === 'video'}
          {#if streaming}
            <span style="width:12px;height:12px;border-radius:2px;background:var(--lost-300);display:inline-block"></span>
            <span>STOP ■</span>
          {:else}
