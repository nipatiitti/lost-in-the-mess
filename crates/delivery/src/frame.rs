...
impl FecFrame {
    pub fn encode(&self) -> Vec<u8> {
        ...
        buf.extend_from_slice(&self.oti.to_be_bytes());
    ...
    pub fn decode(bytes: &[u8]) -> Option<Self> {
        ...
        let oti = u64::from_be_bytes(bytes[4..12].try_into().unwrap());
        let esi = u32::from_be_bytes(bytes[12..16].try_into().unwrap());
        let sym_sz = u16::from_be_bytes(bytes[16..18].try_into().unwrap());
